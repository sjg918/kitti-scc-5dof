...
class CalibNet_base(nn.Module):
    def __init__(self):
        super().__init__()
        md = 4
        use_feat_from = 1

        self.conv6_0 = myconv(1024, 512, kernel_size=3, stride=2)
        self.conv6_1 = myconv(512, 256, kernel_size=3, stride=2)
        self.conv6_2 = myconv(256, 128, kernel_size=3, stride=2)

        self.fc1_left_rot = nn.Linear(640, 4)
        self.fc1_right_rot = nn.Linear(640, 4)


    def forward(self, left, right, left_target, right_target):
        B, C, H, W = left.shape

        cat_feat = torch.cat((left, right), dim=1)
        cat_feat = self.conv6_0(cat_feat)
        cat_feat = self.conv6_1(cat_feat)
        cat_feat = self.conv6_2(cat_feat)

        cat_feat = cat_feat.view(B, -1)
        
        leftout = self.fc1_left_rot(cat_feat)
        rightout = self.fc1_right_rot(cat_feat)
