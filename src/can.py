...
class CrossAttentionNet_visattn(nn.Module):
    def __init__(self, in_channels, qkv_channels):
        super().__init__()
        self.qkv_channels = qkv_channels
    ...
    def match(self, x, y):
        ...
        attn1 = q @ k.permute(0, 2, 1).contiguous()
        attn1_ = F.softmax(attn1, dim=-1)
        attn1 = attn1_ @ y
        attn1 = torch.cat((attn1, x), dim=-1)

        attn2 = k @ q.permute(0, 2, 1).contiguous()
        attn2_ = F.softmax(attn2, dim=-1)
        attn2 = attn2_ @ x
        attn2 = torch.cat((attn2, y), dim=-1)

        attn = torch.cat((attn1, attn2), dim=1)
        attn = attn.permute(0, 2, 1).contiguous().view(B, C*4, H, W)
        return attn, attn1_, attn2_

    def forward(self, left, right):
        B, C, H, W = left.shape

        attnvol0, attn1_, attn2_ = self.match(left, right)
        return attn1_, attn2_
